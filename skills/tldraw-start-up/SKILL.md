...
This is a startup and connection companion to the separately installed official `tldraw-offline` skill. Read that skill first for canvas operations, editor API usage, document scripts, shape safety, and verification. This skill owns only startup and bridge diagnosis.
...
3. Verify `GET /readme`, then make an authenticated `POST /api/search` request returning `await api.getDocs()`. Use `api.getDocs()` for startup discovery; detailed diagnostics may use `api.getFocusedDoc()` and return `{ connected: true, doc }`.
