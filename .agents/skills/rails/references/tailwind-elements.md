...
**When to use**: Build inline, purpose-built dialogs like this for confirmations, info, and interactive dialogs. For bulk-wiring destructive actions (delete links), a global Turbo confirm dialog is available via `data-turbo-confirm` — see [Dialog Patterns in views-hotwire.md](views-hotwire.md#dialog-patterns).
...
For the complete component API (events, methods, attributes, CSS variables) see [tailwind-plus-elements-api.md](tailwind-plus-elements-api.md).
