---
name: rails
description: "Build Rails applications following modern Rails conventions: importmap-rails, Propshaft, Hotwire (Turbo + Stimulus), Devise authentication, concerns over service objects, SQLite default, Solid Queue, Tailwind CSS v4. Use this skill whenever building new Rails applications, adding features to Rails apps, setting up Rails project structure, implementing authentication, implementing authorization and access control, creating models with concerns, adding real-time features, writing tests, configuring background jobs, or any Rails development task. Trigger this even if the user just says 'add a model' or 'set up auth' or 'make this real-time' — any Rails work benefits from these conventions."
---
...
Build Rails applications with modern conventions — server-rendered HTML with Hotwire for interactivity, concerns over service objects, SQLite by default, and no unnecessary dependencies.
...
4. **Devise for authentication** — Devise handles registration, login, password resets, and session management out of the box. It's battle-tested, well-documented, and avoids reinventing security-critical code.
...
**Don't hand-roll authentication** — use Devise. Authentication is security-critical and Devise handles edge cases (timing attacks, password hashing, session fixation) that are easy to get wrong.
...
**Don't write raw HTML inputs inside forms** — use Rails form helpers (`f.check_box`, `f.radio_button`, `f.text_field`, etc.) which handle name/id generation, boolean attributes, and model state automatically. Only use raw `<input>` tags outside a form builder context, and even then never duplicate elements for boolean attributes — use `<%= "checked" if checked %>`. See [references/views-hotwire.md](references/views-hotwire.md).
...
### Controllers & Routing
Read [references/controllers.md](references/controllers.md) for thin controllers, Devise integration, CRUD-oriented routing, scoping concerns, `params.expect`, rate limiting.
...
Read [references/sources.md](references/sources.md) for the three Basecamp/37signals open-source apps (Writebook, Campfire, Fizzy) these patterns are distilled from. Full source is in `../skip-samples/` for deeper research.
